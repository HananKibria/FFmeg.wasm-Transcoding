var createFFmpegCore = (() => {
  ...
  return (
    function (createFFmpegCore = {}) {
      ...
      function freePtrs(ptrArray, len) {
        for (let i = 0; i < len; i++) {
            // Retrieve each string pointer from the pointer array
            const strPtr = Module["getValue"](ptrArray + SIZE_I32 * i, "i32");
            Module["_free"](strPtr); // Free each string
        }
       Module["_free"](ptrArray); // Finally, free the pointer array
    }
      ...
      //  const accessHandle = await draftHandle.createSyncAccessHandle();
        ...
        try {
          ...
          Module["_ffmpeg"](args.length, ptrArray)
      ...
      function spawnThread(threadParams) {
        ...
        threads.push(threadParams.pthread_ptr)
        ...
        stream_ops: {
          ...
          write: function (stream, buffer, offset, length, position, canOwn) {
            if(flagConcat){
              ...
              if (buffer.subarray && node.usedBytes===0){
                if (canOwn) {
                  //node.contents = buffer.subarray(offset, offset + length);
                  writable.write(buffer.subarray(offset, offset + length),{at:0}); 
                //  if(!seeking) stream.position+=length // Take ownership
                  node.usedBytes = length;
                  return length;
                }
               else if (position === 0 && node.usedBytes==0 && flagConcat) {
                //  node.contents = buffer.slice(offset, offset + length);
                  writable.write(buffer.slice(offset, offset + length), { at: 0 });
                //  if(!seeking) stream.position+=length
                 node.usedBytes = length;
                 return length;
               }
               else if (position + length <= node.usedBytes) {
               // node.contents=new Uint8Array(buffer.subarray(offset, offset + length));
                writable.write(buffer.subarray(offset,offset+length),{at:position})
              //  if(!seeking) stream.position+=length
                 node.usedBytes = length;
                return length
               }
      ...
      var ___errno_location = function () {
        return (___errno_location = Module["asm"]["Ka"]).apply(null, arguments)
      };
      var _ntohs = function () {
        return (_ntohs = Module["asm"]["La"]).apply(null, arguments)
      };
      var _htons = function () {
        return (_htons = Module["asm"]["Ma"]).apply(null, arguments)
      };
      var _ffmpeg = Module["_ffmpeg"] = function () {
        return (_ffmpeg = Module["_ffmpeg"] = Module["asm"]["Na"]).apply(null, arguments)
      };
